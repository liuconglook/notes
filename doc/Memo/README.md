...
创建事物的纯粹快乐，以及帮助他人获得的成就感。编程还能唤醒每个人内心的情感。

烦恼来自追求完美、来自他人的设定、来自于过时的设计。
...
- 错误的假设
  - 一切都将运行良好， 每一项任务仅花费它所“应该”花费的时间。

用人月作为衡量一项工作的规模时一个危险和带有欺骗性的神话。

人数和时间的互换仅仅适用于以下情况：

- 任务可分解，且不需要相互的交流。

需要交流的可分解任务：错综复杂关系下的一种实践，沟通、交流的工作量非常大，它很快会消耗任务分解所节省下来的个人时间。从而，添加更多的人手，实际上是延长了而不是缩短了时间进度。
...
很少项目允许为测试分配一半的时间，而测试实际上是花费了进度中一半的时间。

向进度落后的项目中增加人手，只会使进度更加落后。

> 效率

研究表明，效率高和效率低的实施者之间个体差异非常大，经常能够给达到数量级的水平。

> 设计

概念的完整性：要求设计必须由一个人，或者非常少数互有默契的人员来实现。

必须将体系机构同实现区分开来：体系结构陈述的是发生了什么，而实现描述的是如何实现。

新的概念经常来自实现人员或者用户。系统的概念完整性决定了其使用的容易程度，不能与系统基本概念进行整合的良好想法和特色，应该放到一边。如果出现很多非常重要但不兼容的构象，就应该抛弃原来的设计，对不同基本概念进行合并，重新开始。

问题：

- 功能过于繁多，而实际情况中的成本考虑比较少。
- 结构师获得了所有创建发明的快乐，剥夺了很多实现人员的创造力。
- 当体系结构的队伍缓慢工作时，很多实现人员只能空闲地坐着等待。

具体实现中创造和发明的机会，并不会因为指定了外部技术说明而大为减少，相反创造性活动会因为规范化而得到增强。
...
- 牢记是开发人员承担创造性和发明性的实现责任，只能建议，而不能支配。
- 时刻准备着为所指定的说明建议一种实现方法，同样准备接受其他任何能达到目标的方法。
- 对于上述的建议保持低调和不公开。
- 准备放弃坚持所做的改进建议。
...
- 会议。（进行技术陈述）
- 工作手册。
  - 是对项目必须产出的一系列文档进行组织的一种结构。
  - 在仔细定义规定什么的同时，定义未规定什么。


> 减少交流

人力划分（division of labor）

限定职责范围（specialization of function）
...
普遍的做法是，选择一种方法，试试看；如果失败了，没关系，再试试别的方法。不管怎么样，重要的是先尝试。
